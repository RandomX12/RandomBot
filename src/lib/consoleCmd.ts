...
type RuntimeCMD<T extends CommandType> = {
    input : string,
    description? : string,
    fn :  T extends "SYNC" ? (args : ArgRuntimeCMD[])=> void : (args : ArgRuntimeCMD[])=> Promise<void>,
    type? : T,
...
export function addToHistory<S extends CMDStatus>(cmd : Omit<RuntimeCMDHistory<S>,"time" | "command" | "date">){
    history.push({
...
export async function convertArg<T extends ArgType>(arg : string,type? : T) : 
Promise<ReturnValue<T>>
{
    ...
    if(type === "arr"){
        ...
        try{
            ...
            error(`error when converting arg "${arg}" to javascript array : \n${err.message}`)
            }
    }
}
...
export default function listenToCmdRunTime(){
    ...
    rl.on('line', async(input) => {
        ...
        try{
                    ...
                    if(e.args.length > 0){
                        for(let i = 0;i< e.args.length;i++){
                            ...
                            if(e.args[i].type){
                                ...
                                if(!check){
                                    error(`TypeError : Type of arg ${i} is ${e.args[i].type} but got ${typeOfArg(args[i])} ("${args[i]}")`)
                                    return
                                }
                            }
                            try{
                                ...
                                throw new Error(`ConvertingError : error when converting arg ${i} "${args[i]}" to javascript ${e.args[i].type} :\n${err.message}`)
                            }
            ...
            if(!found){
                throw new Error(`"${input.split(" ")[0]}" runtime command is not found. if you want to create a new runtime command use the addRuntimeCMD function from ${__filename}`)
            }
            addToHistory({
                input : input,
                status : "SUCCESSFUL"
    ...
    if(args.length > 1){
        ...
        args.map((e,i)=>{
            if(regex.test(e)){
                ...
                regex.test(e)
                //@ts-ignore
                const propName =  e.replaceAll(/\[:|\]|\?|(:(bol|str|int|float|arr|txt))$/g,"")
            ...
            throw new Error(`Invalid Runtime command syntax at arg ${i} "${e}" ${command.input} command`)
        })
    }
    ...
    fn(args) {
        if(!args[0]){
            const table = commands.map((e)=>{
                let argsStr = ""
                for(let i = 0;i<e.args.length;i++){
                    argsStr += `(${e.args[i].name} ${e.args[i].type})${e.args[i].required ? "" : "?"}${i + 1 === e.args.length ? "" : " "}`
                }
                return{
                    name : e.input,
